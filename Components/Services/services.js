...
const showTalentDetails_services = (entertainer) => {
...
const loadServices = () => {
  ...
  fetch("http://localhost:3000/api/entertainers", {
    ...
    .then(data => {
let entertainers = (data.entertainers || []).filter(ent => ent.role === "services");
      ...
      if (entertainers.length === 0) {
        talentGrid.innerHTML = `<p>No services found.</p>`;
        return;
      }
      ...
      entertainers.forEach(entertainer => {
        ...
        card.addEventListener("click", () => {
          showTalentDetails_services(entertainer);
        });
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  console.log("easyhire.js: DOM already loaded, running loadServices");
  loadServices();
}