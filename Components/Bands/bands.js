...
const showTalentDetails_loadBands2 = (entertainer) => {
...
const loadBands = () => {
  ...
  fetch("http://localhost:3000/api/entertainers", {
    ...
    .then(data => {
let entertainers = (data.entertainers || []).filter(ent => ent.role === "bands");
      ...
      if (entertainers.length === 0) {
        talentGrid.innerHTML = `<p>No Bands found.</p>`;
        return;
      }
      ...
      entertainers.forEach(entertainer => {
        ...
        card.addEventListener("click", () => {
          showTalentDetails_loadBands2(entertainer);
        });
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  console.log("easyhire.js: DOM already loaded, running loadBands");
  loadBands();
}