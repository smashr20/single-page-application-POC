...
const showTalentDetails_cele = (entertainer) => {
...
const loadCelebrities = () => {
  ...
  fetch("http://localhost:3000/api/entertainers", {
    ...
    .then(data => {
let entertainers = (data.entertainers || []).filter(ent => ent.role === "celebrities");
      ...
      if (entertainers.length === 0) {
        talentGrid.innerHTML = `<p>No celebrities found.</p>`;
        return;
      }
      ...
      entertainers.forEach(entertainer => {
        ...
        card.addEventListener("click", () => {
          showTalentDetails_cele(entertainer);
        });
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  console.log("easyhire.js: DOM already loaded, running loadCelebrities");
  loadCelebrities();
}