...
const showTalentDetails_speaker = (entertainer) => {
...
const loadEntertainers_speaker = () => {
  ...
  fetch("http://localhost:3000/api/entertainers", {
    method: "GET",
    headers: { "Content-Type": "application/json" }
  })
    .then(response => {
    ...
    .then(data => {
let entertainers = (data.entertainers || []).filter(ent => ent.role === "speakers");
      ...
      if (entertainers.length === 0) {
        talentGrid.innerHTML = `<p>No speakers found.</p>`;
        return;
      }
      ...
      entertainers.forEach(entertainer => {
        ...
        card.addEventListener("click", () => {
          showTalentDetails_speaker(entertainer);
        });
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  console.log("easyhire.js: DOM already loaded, running loadEntertainers_speaker");
  loadEntertainers_speaker();
}